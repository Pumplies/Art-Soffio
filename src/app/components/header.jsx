...
export default function Header() {
  ...
  return (
    <>
      <header className="flex justify-between items-center py-[15px] px-[40px] xl:px-[138px] bg-[#F3FBFF] w-full fixed top-0 z-50">
        <button className={` order-0  ${isNarrowScreen ? "block" : "hidden"}`} onClick={toggleBurgerMenu} aria-label="Toggle Menu">
          <span className="block w-6 h-0.5 bg-black mb-1"></span>
          <span className="block w-6 h-0.5 bg-black mb-1"></span>
          <span className="block w-6 h-0.5 bg-black"></span>
        </button>
        <Image src="/logo.png" alt="Art Soffio Logo" width={370} height={70}  onDragStart={(e) => e.preventDefault()} priority className="w-[188px] h-[35px] mx-auto md:w-[280px] md:h-[53px] hd:w-[370px] hd:h-[70px] xl:mx-0"></Image>
